    ...
    with open(self.src, encoding="utf-8") as f:
      for line in f:
        ...
        if r:
          ...
          filename = self.dist + '{0:04}'.format(count) +"."+r[0] + ".txt"
          print(filename)
...
if __name__ == '__main__':
  ...
  utf_filename= os.path.splitext(filename)[0] + "-uft8"+ os.path.splitext(filename)[1]
  if os.path.exists(utf_filename) and os.path.getsize(utf_filename) > 0:
    filename = utf_filename
  cf = CutFile(filename, os.path.splitext(filename)[0], "^第.*章")
  cf.cut()
